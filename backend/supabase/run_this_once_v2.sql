-- 1. DELETE ANY OLD CORRUPTED USERS
...
DELETE FROM public.menu_items CASCADE;
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;

-- 2. INSERT ACCOUNTS INTO AUTH
-- (Removed deprecated older columns like is_super_admin to be safe on modern Supabase)
INSERT INTO auth.users (id, aud, role, email, encrypted_password, email_confirmed_at, created_at, updated_at, raw_app_meta_data, raw_user_meta_data)
VALUES 
  ('11111111-1111-1111-1111-111111111111', 'authenticated', 'authenticated', 'admin@quickserve.com', crypt('password123', gen_salt('bf')), now(), now(), now(), '{"provider": "email", "providers": ["email"]}', '{}'),
  ('22222222-2222-2222-2222-222222222222', 'authenticated', 'authenticated', 'waiter@quickserve.com', crypt('password123', gen_salt('bf')), now(), now(), now(), '{"provider": "email", "providers": ["email"]}', '{}'),
  ('33333333-3333-3333-3333-333333333333', 'authenticated', 'authenticated', 'kitchen@quickserve.com', crypt('password123', gen_salt('bf')), now(), now(), now(), '{"provider": "email", "providers": ["email"]}', '{}'),
  ('44444444-4444-4444-4444-444444444444', 'authenticated', 'authenticated', 'counter@quickserve.com', crypt('password123', gen_salt('bf')), now(), now(), now(), '{"provider": "email", "providers": ["email"]}', '{}');

-- 3. LINK IDENTITIES (THIS IS WHAT STOPPED THE 500 SCHEMA ERROR)
